...
export default function TeacherProfile() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const fallbackProfile: TeacherProfile = {
          ...
          displayName: user?.displayName || 'Teacher Name',
          firstName: 'Teacher',
          lastName: 'Name',
          avatar: '',
  ...
  const loadStats = async () => {
    try {
      ...
      if (quizzesResponse.ok && classesResponse.ok && attemptsResponse.ok) {
        const quizzes = await quizzesResponse.json();
        const classes = await classesResponse.json();
        const attempts = await attemptsResponse.json();
        
        const teacherQuizIds = quizzes.map((q: any) => q.id);
        const teacherAttempts = attempts.filter((a: any) => teacherQuizIds.includes(a.quizId));
        ...
        const enrollmentsPromises = classes.map((cls: any) =>
        ...
        const enrollmentsData = await Promise.all(enrollmentsPromises);
        const totalStudents = enrollmentsData.reduce((sum, data) => sum + (data.totalStudents || 0), 0);
  ...
  const handleInputChange = (field: keyof TeacherProfile | string, value: any) => {
    if (!editedProfile) return;
    ...
    if (field.includes('.')) {
      ...
      if (subChild) {
        setEditedProfile({
          ...editedProfile,
          [parent]: {
            ...editedProfile[parent as keyof TeacherProfile],
            [child]: {
              ...(editedProfile[parent as keyof TeacherProfile] as any)[child],
              [subChild]: value
            }
          }
        });
            ...
            <div className="flex-1 space-y-2">
              {isEditing ? (
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const lastName = editedProfile?.lastName || '';
                        handleInputChange('displayName', `${firstName} ${lastName}`.trim());
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const firstName = editedProfile?.firstName || '';
                        handleInputChange('displayName', `${firstName} ${lastName}`.trim());
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">{profile.department || 'Teacher'}</p>
                </div>
              )}
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your teaching accomplishments</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-3 md:grid-cols-2">
                <div className="flex items-center gap-3 p-3 border rounded-lg">
                  ...
                  <div>
                    <p className="font-medium">Quiz Creator</p>
                    <p className="text-sm text-muted-foreground">Created {stats.totalQuizzes} quizzes</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 border rounded-lg">
                  <Users className="h-8 w-8 text-blue-500" />
                  <div>
                    <p className="font-medium">Class Manager</p>
                    <p className="text-sm text-muted-foreground">Managing {stats.totalClasses} classes</p>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              <div>
                ...
                <div className="space-y-3 mt-2">
                  {[
                    ...
                    { key: 'showClasses', label: 'Show classes publicly' }
                  ].map(({ key, label }) => (
                    <div key={key} className="flex items-center justify-between">
                      <span>{label}</span>
                      {isEditing ? (
